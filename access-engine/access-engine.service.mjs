...
async function pendenciaFinanceiraAluno(aluno = {}) {
  ...
  const [mensalidades, financeiro] = await Promise.all([
    ...
    repo.listarFinanceiro()
  ]);
  const pendencias = [...mensalidades, ...financeiro]
...
function bloqueioManualAluno(aluno = {}, matricula = {}) {
  return aluno.bloqueado === true ||
    aluno.bloqueioCheckin === true ||
    matricula.bloqueada === true ||
    matricula.bloqueioCheckin === true ||
    statusBloqueado(matricula.status);
}

async function encontrarMatriculaAtivaAluno(aluno = {}) {
  const alunoId = String(aluno.id || aluno._id || aluno.alunoId || '');
  const matriculaId = String(aluno.matriculaId || '');
  const matriculas = await repo.listarMatriculas();
  return matriculas
    .filter((matricula) =>
      (matriculaId && String(matricula.id || '') === matriculaId) ||
      (alunoId && String(matricula.alunoId || matricula.aluno_id || '') === alunoId)
    )
    .filter((matricula) => statusAtivo(matricula.status))
    .sort((a, b) => String(b.atualizadoEm || b.criadoEm || b.dataMatricula || '').localeCompare(String(a.atualizadoEm || a.criadoEm || a.dataMatricula || '')))[0] || null;
}
...
async function executarAvaliacao({ aluno, identificador = '', dispositivoId = '', direcao = 'entrada', origem = 'simulador' } = {}) {
  ...
  const matriculaAtiva = aluno ? await encontrarMatriculaAtivaAluno(aluno) : null;
  ...
  if (autorizado && aluno && bloqueioManualAluno(aluno, matriculaAtiva)) {
    autorizado = false;
    motivo = 'Aluno ou matrícula com bloqueio manual';
  }
  ...
  // Somente depois de confirmar matrícula ativa, ausência de bloqueio manual e
  // inexistência de dívida vencida, corrige os campos contraditórios que ficaram
  // gravados pela reativação antiga. Assim app, check-in e catraca passam a ler o
  // mesmo estado sem liberar aluno pendente ou inadimplente.
  if (autorizado && aluno) {
    aluno = await repo.sincronizarAlunoLiberado(aluno, matriculaAtiva);
  }
