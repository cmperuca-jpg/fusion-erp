...
function possuiPendenciaVencidaDoAluno(alunoId, ...fontes) {
  return fontes.flat().some((item) =>
    idAluno(item) === txt(alunoId) &&
    !finalizado(item) &&
    !quitado(item) &&
    saldoC(item) > 0 &&
    Boolean(dataVencimento(item)) &&
    dataVencimento(item) < hoje()
  );
}
    ...
    for (const a of alocacoes) {
      ...
      if (deveAtivarMatricula) {
        ...
        if (mi >= 0 && !["cancelada", "encerrada"].includes(status(matriculas[mi]))) {
          ...
          const possuiPendenciaVencida = possuiPendenciaVencidaDoAluno(alunoUnico, titulos, mensalidades);
          if (ai >= 0) alunos[ai] = {
            ...alunos[ai],
            status: "ativo",
            situacao: "ativo",
            ...
            inadimplente: possuiPendenciaVencida,
            emAtraso: possuiPendenciaVencida,
            motivoBloqueio: "",
            motivoBloqueioCheckin: "",
            reativacaoPendenteEm: "",
            recebimentoReativacaoId: "",
            liberadoAcessoEm: agora(),
            atualizadoEm: agora()
          };
        }
      }
