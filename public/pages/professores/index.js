...
function fechar(){ $('#modal').classList.add('hidden'); $('#form').reset(); $('#id').value=''; documentos=[]; renderDocs(); $('#prontuario').textContent='Selecione um professor para carregar o prontuário.'; trocarTab('cadastro'); }
...
function preencher(p){$('#id').value=idProfessor(p);if($('#senha'))$('#senha').value='';if($('#confirmarSenha'))$('#confirmarSenha').value='';['nome','status','cpf','rg','cref','email','telefone','whatsapp','dataNascimento','endereco','especialidade','horarioInicio','horarioFim','tipoContrato','valorHora','banco','agencia','conta','chavePix','observacoes'].forEach(k=>{const el=$(`#${k}`);if(el)el.value=p[k]||'';});$('#especialidades').value=joinLista(p.especialidades);$('#modalidades').value=joinLista(p.modalidades);$('#diasTrabalho').value=joinLista(p.diasTrabalho);documentos=Array.isArray(p.documentos)?p.documentos:[];renderDocs();}
...
async function salvar(ev){ev.preventDefault();const id=$('#id').value;const senha=$('#senha')?.value||'';const confirmar=$('#confirmarSenha')?.value||'';if(!id&&!senha)return mostrar('Informe a senha do professor para acesso ao portal.','erro');if(senha!==confirmar)return mostrar('A confirmação da senha não confere.','erro');if(senha&&senha.length<6)return mostrar('A senha deve ter pelo menos 6 caracteres.','erro');const dados=coletar();if(!dados.nome||dados.nome.length<3)return mostrar('Informe o nome completo.','erro');try{const resp=await fetch(id?`${API}/${encodeURIComponent(id)}`:API,{method:id?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(dados)});const j=await safeJson(resp);if(!resp.ok||j.ok===false)throw new Error(j.mensagem||`HTTP ${resp.status}`);fechar();mostrar(id?'Professor atualizado.':'Professor cadastrado.','sucesso');await carregar();}catch(e){mostrar(e.message,'erro');}}
...
$('#btnNovo').addEventListener('click',()=>{fechar();abrirModal();});$('#btnAtualizar').addEventListener('click',carregar);$('#btnFechar').addEventListener('click',fechar);$('#btnCancelar').addEventListener('click',fechar);$('#form').addEventListener('submit',salvar);$('#busca').addEventListener('input',render);$('#filtroStatus').addEventListener('change',render);$('#btnAdicionarDoc').addEventListener('click',adicionarDoc);document.querySelectorAll('.tab').forEach(b=>b.addEventListener('click',()=>trocarTab(b.dataset.tab)));
carregar();
