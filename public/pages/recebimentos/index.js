...
function atualizarResumo(){const r=estado.registros;$("#kpiTotal").textContent=r.length;$("#kpiAbertos").textContent=r.filter(x=>statusItem(x)==="aberto"&&estaEmAberto(x)).length;$("#kpiRecebidos").textContent=r.filter(x=>statusItem(x)==="recebido").length;$("#kpiParciais").textContent=r.filter(x=>statusItem(x)==="parcial").length;$("#kpiVencidos").textContent=r.filter(estaVencido).length;$("#kpiValorBruto").textContent=moeda(r.reduce((a,x)=>a+valorBruto(x),0));$("#kpiValorLiquido").textContent=moeda(r.reduce((a,x)=>a+valorRecebido(x),0));$("#kpiValorAberto").textContent=moeda(r.reduce((a,x)=>estaEmAberto(x)?a+saldoItem(x):a,0))}
...
function abrirLote(){const itens=selecionadosBaixaveis();if(!itens.length)return;$("#loteQtd").textContent=`${itens.length} título${itens.length===1?"":"s"} selecionado${itens.length===1?"":"s"}`;$("#loteValor").textContent=`${moeda(valorSelecionado())} disponível para baixa`;$("#loteForma").value="pix";$("#loteObs").value="Baixa em lote pela tela de recebimentos";$("#modalLote").showModal()}
async function confirmarLote(){const itens=selecionadosBaixaveis(),forma=$("#loteForma").value,observacao=$("#loteObs").value;let ok=0,falhas=0;for(const item of itens){const id=idRegistro(item);try{const valor=saldoBaixavel(item);await baixarRecebimento(id,{valor,valorPago:valor,valorRecebido:valor,valorBaixa:valor,desconto:0,acrescimo:0,juros:0,formaPagamento:forma,forma,observacao});estado.selecionados.delete(String(id));ok++}catch{falhas++}}$("#modalLote").close();await carregar();if(falhas)alert(`Baixa em lote concluída parcialmente. Sucesso: ${ok}. Falhas: ${falhas}.`)}
