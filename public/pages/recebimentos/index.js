import { listarRecebimentos, criarRecebimento, estornarRecebimento, cancelarRecebimento, obterBaseAtiva, diagnosticarRecebimentos } from "./api.js?v=20260828-estorno-real-1";
...
if (MODO_PAGAMENTO_EMBUTIDO) document.documentElement.classList.add("recebimentos-embed");
...
/*
 * Status operacional da tela.
 * Um título programado NÃO é "em aberto", mas continua disponível para baixa antecipada.
 *
 * A heurística de recuperação abaixo cobre registros legados/API que chegam como "aberto"
 * apesar de serem futuros e terem saldo contábil zerado. Isso evita reclassificar como
 * programado um título futuro realmente aberto, pois nesse caso o saldo contábil deve ser > 0.
 */
function statusItem(item){
  const atual=normalizarStatus(item?.status);
  const statusAlternativo=normalizar(campoTexto(item,[
    "statusFinanceiro","statusOriginal","situacao","estado","situacaoFinanceira"
  ],""));
  const marcadoProgramado=
    atual==="programado" ||
    statusAlternativo==="programado" ||
    item?.programado===true ||
    item?.agendado===true ||
    item?.cobrancaProgramada===true ||
    item?.recorrenciaProgramada===true;

  if(marcadoProgramado)return"programado";

  if(atual==="aberto"){
    const data=dataItem(item);
    const saldoPersistido=campoValor(item,["valorRestante","saldo","valorAberto"],null);
    const futuro=Boolean(data&&data>hojeIso());
    const semRecebimento=valorRecebidoBruto(item)<=0.009;
    const saldoContabilZerado=saldoPersistido!==null&&Number(saldoPersistido)<=0.009;
    if(futuro&&semRecebimento&&saldoContabilZerado&&valorBruto(item)>0){
      return"programado";
    }
  }
...
/* Saldo contábil usado em KPIs de "em aberto": programado nunca entra. */
function estaEmAberto(item){return["aberto","parcial"].includes(statusItem(item))&&saldoItem(item)>0}

/* Valor que pode ser efetivamente baixado. Programado pode ser antecipado. */
function saldoBaixavel(item){
  if(estaProgramado(item)){
    return Math.max(0,Number((valorBruto(item)-valorRecebidoBruto(item)).toFixed(2)));
  }
  return Math.max(0,saldoItem(item));
}
function estaBaixavel(item){
  return["aberto","parcial","programado"].includes(statusItem(item))&&saldoBaixavel(item)>0;
}
function saldoExibido(item){return estaProgramado(item)?saldoBaixavel(item):saldoItem(item)}

function entraNaPrevisaoCaixa30Dias(item){
  const st=statusItem(item);
  if(!["aberto","parcial","programado"].includes(st))return false;
  const data=dataItem(item);
  if(!data)return false;
  const inicio=hojeIso();
  const fim=somarDiasIso(inicio,30);
  return data>=inicio&&data<=fim&&saldoBaixavel(item)>0;
}
function previsaoCaixa30Dias(lista=[]){
  return lista.reduce((total,item)=>entraNaPrevisaoCaixa30Dias(item)?total+saldoBaixavel(item):total,0);
}
...
function valorSelecionado(){return selecionadosBaixaveis().reduce((a,i)=>a+saldoBaixavel(i),0)}
...
function valorSort(item,c){return{vencimento:dataItem(item),cliente:campoTexto(item,["cliente","aluno","nomeCliente","responsavel"],""),descricao:campoTexto(item,["descricao","observacao","documento","referencia"],""),forma:campoTexto(item,["formaPagamento","forma","meioPagamento","pagamento"],""),valor:valorBruto(item),recebido:valorRecebido(item),saldo:saldoExibido(item),status:statusItem(item)}[c]??""}
...
async function confirmarEstorno(id){const motivo=prompt("Motivo do estorno:","Estorno solicitado pelo usuário");if(motivo===null)return;const motivoNormalizado=String(motivo||"").trim();if(motivoNormalizado.length<3){alert("Informe o motivo do estorno com pelo menos 3 caracteres.");return}if(!confirm("ATENÇÃO: se este recebimento foi feito por PIX ou cartão integrado, o Fusion solicitará a DEVOLUÇÃO REAL do dinheiro ao gateway. Esta ação pode ser irreversível. Confirma o estorno?"))return;const operacaoId=`estorno-real-${id}-${crypto.randomUUID?.()||Date.now()}`;try{const resultado=await estornarRecebimento(id,{motivo:motivoNormalizado,operacaoId});estado.selecionados.delete(String(id));const externo=resultado?.reembolsoExterno;const pendenteExterno=Boolean(externo?.aguardandoConfirmacao||resultado?.pendente);if(pendenteExterno){alert(`A devolução foi solicitada ao ${String(externo?.provider||"gateway").toUpperCase()} e ainda está em processamento. O recebimento continuará como RECEBIDO no Fusion até a confirmação real.`)}else if(externo?.necessario&&externo?.devolucaoAceita){alert(`Devolução confirmada pelo ${String(externo.provider||"gateway").toUpperCase()} e estorno registrado no Fusion.${externo.protocolo?`\nProtocolo: ${externo.protocolo}`:""}`)}else{alert("Estorno registrado no Fusion.")}if(!pendenteExterno&&$("#fStatus")?.value==="recebido")$("#fStatus").value="";estado.pagina=1;await carregar()}catch(e){alert(erroAmigavel(e))}}
...
function abrirResumo(){
  ...
  const porStatus=agruparResumo(lista,(i)=>statusItem(i));
...
function exportarResumoCsv(){
  aplicarFiltrosLocais();
  const porStatus=agruparResumo(estado.filtrados,(i)=>statusItem(i));
...
async function abrirBaixaAutomaticaPorUrl(){
  ...
  if(mensalidadeId&&origem.startsWith("dashboard")){
    ...
    try{
      ...
      if(MODO_PAGAMENTO_EMBUTIDO)destino.set("embed","1");
