...
function preencherSelectPlanos(valorAtual = "") {
  ...
  select.innerHTML = '<option value="">Sem plano agora</option>' + planos.map((plano) => {
...
async function salvarAluno(ev) {
  ...
  try {
    ...
    let integracao = null;
    if (!id) {
      integracao = await integrarMatriculaAposCadastro(salvo, dados);
      registrarHistoricoLocal(idSalvo, "matricula", `Matrícula criada: ${integracao?.matricula?.numero || "sem número"}`);
    }

    fecharModal();
    mostrarAlerta(id ? "Aluno atualizado com sucesso." : "Aluno cadastrado, matriculado e lançado no financeiro.", "sucesso");
    await carregarAlunos();

    if (!id && integracao && confirmarRecebimentoAgora(integracao)) {
      abrirRecebimentoMatricula(integracao);
    }
  } catch (erro) {
    mostrarAlerta(erro.message, "erro");
  } finally {
...
function coletarDadosFormulario() {
  ...
  if (!dados.data_matricula) dados.data_matricula = dataHojeISO();
...
function validarAluno(d) {
  ...
  if (!d.plano) return "Selecione um plano cadastrado.";
