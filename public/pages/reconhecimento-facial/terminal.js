(function(){
  ...
  async function conectar(){const token=$("#token").value.trim(),id=$("#terminalId").value.trim()||"entrada-principal";localStorage.setItem("fusionFacialToken",token);localStorage.setItem("fusionFacialTerminal",id);try{await api("/api/reconhecimento-facial/terminal/status");$("#config").classList.add("hidden");await iniciarCamera();navigator.wakeLock?.request("screen").catch(()=>{});ciclo();}catch(e){$("#configMsg").textContent=e.message;$("#configMsg").className="face-message error";}}
  $("#conectar").addEventListener("click",conectar);$("#telaCheia").addEventListener("click",()=>document.documentElement.requestFullscreen?.());$("#terminalId").value=cfg.id;if(cfg.token){$("#token").value=cfg.token;conectar();}
})();
