...
const ARQUIVO = "professores.json";
...
async function salvar(lista) {
  ...
  await salvarJsonDuravel(ARQUIVO, dados);
  return dados;
}
...
export function verificarSenha(senha, hashSalvo = "") {
  ...
  try {
    return crypto.timingSafeEqual(Buffer.from(hash), Buffer.from(novo));
  } catch {
    return false;
  }
}
...
export async function buscarProfessorPorIdentificador(identificador) {
  ...
  return (
    lista.find((p = {}) => {
      const textos = [p.login, p.email, p.nome, p.cref, p.id]
        .map(normalizar)
        .filter(Boolean);

      const numeros = [p.cpf, p.telefone, p.whatsapp, p.celular]
        .map(limparCpf)
        .filter(Boolean);

      return (
        textos.includes(alvo) ||
...
export async function criarProfessor(dados) {
  ...
  if (cpf && lista.some((p) => limparCpf(p.cpf) === cpf)) {
    throw new Error("Já existe professor cadastrado com este CPF.");
  }

  const senhaAcesso = dados.senha
    ? String(dados.senha)
    : dados.senhaAcesso || dados.senhaPortal || "";
...
export async function atualizarProfessor(id, dados) {
  ...
  if (cpf && lista.some((p, i) => i !== idx && limparCpf(p.cpf) === cpf)) {
    throw new Error("Já existe outro professor cadastrado com este CPF.");
  }
...
export async function alterarStatusProfessor(id, status) {
  ...
  const novoStatus = ["ativo", "desbloqueado"].includes(statusNormalizado)
    ? "Ativo"
    : "Bloqueado";
