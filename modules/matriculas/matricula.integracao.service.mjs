...
import { matriculaBloqueiaNovaMatricula, normalizarStatusMatricula } from './matricula-status.util.mjs';
...
function matriculaAtivaDoAluno(matriculas, alunoId){ return (matriculas||[]).find(m=>String(m.alunoId)===String(alunoId)&&matriculaBloqueiaNovaMatricula(m)); }
...
export async function listarMatriculas(filtros={}){ const base=await carregarBaseMatricula(); const termo=normalizar(filtros.q||filtros.busca||''); const status=normalizarStatusMatricula(filtros.status||''); const alunoId=String(filtros.alunoId||filtros.aluno_id||''); let dados=base.matriculas||[]; if(status && status!=='todos' && status!=='todas') dados=dados.filter(m=>normalizarStatusMatricula(m.status)===status); if(alunoId) dados=dados.filter(m=>String(m.alunoId)===alunoId); if(termo) dados=dados.filter(m=>normalizar([m.numero,m.aluno,m.plano,m.turma,m.status].join(' ')).includes(termo)); return { ok:true, success:true, total:dados.length, dados:dados.map(m=>resumirMatricula(m)).sort((a,b)=>String(b.criadoEm||'').localeCompare(String(a.criadoEm||''))) }; }
...
export async function integrarMatriculaAluno(alunoId, planoId, opcoes={}){
  ...
  const payloadChk={ tipo:'vinculo_matricula', alunoId:aluno.id, aluno:nome, matriculaId:matricula.id, numeroMatricula:matricula.numero, planoId:plano.id || '', plano:plano.nome || 'SEM PLANO', tipoPlano:tipo, turmaIds:matricula.turmaIds, turma:matricula.turma, modalidade:matricula.modalidade, professor:matricula.professor, horario:matricula.horario, sala:matricula.sala, turmas:servicos, servicos, status:'Bloqueado', atualizadoEm:agoraISO() };
...
export async function atualizarTurmasMatricula(id, opcoes={}, usuario='sistema'){
  ...
  if(chk) Object.assign(chk,{ turmaIds:m.turmaIds, turma:m.turma, modalidade:m.modalidade, professor:m.professor, horario:m.horario, sala:m.sala, turmas, servicos:turmas, atualizadoEm:agoraISO() });
