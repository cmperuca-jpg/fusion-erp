...
async function registrarRecebimentoDoDiaMensalidade(mensalidade = {}) {
  const recebimentos = await lerJson(RECEBIMENTOS_FILE, []);
  const recebimentoId = mensalidade.recebimentoId || `rec_${mensalidade.lancamentoFinanceiroId || mensalidade.id}`;
  const idx = recebimentos.findIndex(r =>
    String(r.id || '') === String(recebimentoId) ||
    String(r.mensalidadeId || '') === String(mensalidade.id || '') ||
    String(r.lancamentoFinanceiroId || '') === String(mensalidade.lancamentoFinanceiroId || '')
  );

  const valorBruto = numero(mensalidade.valorPago ?? mensalidade.valorBrutoRecebido ?? mensalidade.valorDevido ?? mensalidade.valor, 0);
  const valorLiquido = numero(mensalidade.valorLiquido ?? valorBruto, valorBruto);
  const recebimento = {
    id: recebimentoId,
    descricao: ehMatriculaInicial(mensalidade)
      ? (mensalidade.descricao || `Matrícula inicial - ${mensalidade.alunoNome || mensalidade.aluno || ''}`)
      : `Mensalidade ${mensalidade.alunoNome || mensalidade.aluno || ''} - ${mensalidade.competencia}`,
    categoria: ehMatriculaInicial(mensalidade) ? 'Matrículas' : 'Mensalidades',
    centroCusto: 'Caixa',
    pessoa: mensalidade.alunoNome || mensalidade.aluno || mensalidade.pessoa || '',
    cliente: mensalidade.alunoNome || mensalidade.aluno || mensalidade.pessoa || '',
    aluno: mensalidade.alunoNome || mensalidade.aluno || mensalidade.pessoa || '',
    alunoId: mensalidade.alunoId || '',
    mensalidadeId: mensalidade.id || '',
    matriculaId: mensalidade.matriculaId || '',
    planoId: mensalidade.planoId || '',
    ...
    valorBruto,
    valorDevido: numero(mensalidade.valorDevido ?? valorBruto, valorBruto),
    valorRecebido: valorBruto,
    valorLiquido,
    valorRestante: 0,
    taxaValor: numero(mensalidade.taxaOperadoraValor, 0),
    taxaPercentual: numero(mensalidade.taxaOperadoraPercentual, 0),
    vencimento: mensalidade.vencimento || hojeISO(),
    dataRecebimento: mensalidade.dataPagamento || mensalidade.pagamento || hojeISO(),
    pagamento: mensalidade.dataPagamento || mensalidade.pagamento || hojeISO(),
    status: 'recebido',
    origem: 'mensalidades',
    caixaId: mensalidade.caixaId || '',
    movimentoCaixaId: mensalidade.movimentoCaixaId || '',
    ...
    confirmadoPor: mensalidade.baixadoPor || 'Administrador',
    confirmadoEm: mensalidade.baixadoEm || agoraISO(),
    atualizadoEm: agoraISO(),
    criadoEm: mensalidade.criadoEm || agoraISO()
  };

  if (idx >= 0) recebimentos[idx] = { ...recebimentos[idx], ...recebimento };
  else recebimentos.push(recebimento);
...
export async function baixarMensalidade(id, dados = {}) {
  ...
  const recebimentoDoDia = await registrarRecebimentoDoDiaMensalidade(paga);
  paga.recebimentoId = recebimentoDoDia.id;
