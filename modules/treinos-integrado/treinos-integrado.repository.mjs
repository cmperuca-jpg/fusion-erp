...
const FILES = {
  ...
  treinosLegado: path.join(DATA_DIR, "treinos.json"),
...
export async function carregarBaseTreinos() {
  return {
    ...
    treinosLegado: await lerJson(FILES.treinosLegado, []),
    biblioteca: await lerJson(FILES.biblioteca, [])
  };
}

export async function salvarTreinos(lista = []) {
  await salvarJson(FILES.treinos, Array.isArray(lista) ? lista : []);
}
