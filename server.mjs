...
import saasRoutes from "./modules/saas/saas.routes.mjs";
import suporteRoutes from "./modules/suporte/suporte.routes.mjs";
...
// Rotas financeiras diretas legadas removidas.
// A persistencia financeira segue exclusivamente modules/financeiro.
...
app.get("/api/sistema/observabilidade", async (req, res) => {
  try {
    res.json(await observabilidadeSistema(req.query || {}));
  } catch (erro) {
    res.status(500).json({
      ok: false,
